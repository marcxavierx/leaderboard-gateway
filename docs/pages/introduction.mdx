...
You need to set up two environment variables for authentication:
...
4. Add the second variable:
   - Click **"Add Variable"**
   - **Name:** `CASINO_COOKIE`
   - **Value:** Your casino API session cookie for authentication

   Example: `token=def456uvw`

   > 💡 **How to get your session cookie:** Log into your casino API dashboard in your browser, open Developer Tools (F12), go to the Network tab, make a request to your API, and copy the `Cookie` header value from the request headers.

5. Click **"Save"**

> 🔒 **Both values stay private.** Only you can see them in your environment variables. The leaderboard developer will never see your casino API credentials or authentication tokens.
...
- ✅ `CASINO_COOKIE` is set in environment variables
...
### Update Casino API Configuration
If your casino API endpoint or authentication changes:

1. Go to **Settings** → **Environment Variables**
2. Click on `CASINO_URL` or `CASINO_COOKIE` to update
3. Update the value
4. Click **"Save"**

Changes take effect immediately - no need to redeploy.

> 💡 **Note:** If your casino session cookie expires, you'll need to update the `CASINO_COOKIE` environment variable with a fresh session token.
...
- Verify that `CASINO_COOKIE` is set and hasn't expired
- Test your casino API directly to confirm it's responding
- Check Zuplo logs in Analytics section for detailed error messages
